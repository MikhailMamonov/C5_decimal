...
int substract_mantissas(s21_decimal value_1, s21_decimal value_2,
                        s21_decimal *result) {
  ...
  for (int i = 0; i < SIGNIFICANT_BYTES; i++) {
    ...
    unsigned long long diff =
        (unsigned long long)value_1_unsigned - value_2_unsigned - borrow;
...
void set_sign(s21_decimal *value, int sign) {
  ...
  value->bits[SIGNIFICANT_BYTES] =
      set_byte(value->bits[SIGNIFICANT_BYTES], SIGN_BYTE_IDX, byte_value);
}

void set_scale(s21_decimal *value, int scale) {
  value->bits[SIGNIFICANT_BYTES] =
      set_byte(value->bits[SIGNIFICANT_BYTES], SCALE_BYTE_IDX, scale);
}
...
int div_by_10(s21_decimal *num) {
  ...
  for (int i = ELDER_BIT; i >= 0; i--) {
    ...
    unsigned int bit =
        ((unsigned int)copy.bits[curr_byte] & bit_mask) >> bit_num;
...
int get_bit_val(s21_decimal value, int idx) {
  ...
  unsigned int bit_mask = 1 << bit_num;
  unsigned int bit =
      ((unsigned int)value.bits[curr_byte] & bit_mask) >> bit_num;
