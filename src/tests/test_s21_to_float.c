...
TO_FLOAT_TEST_CASES(to_float_with_scale, {
  .src = {{0x0012D687, 0x00000000, 0x00000000, 0x00070000}}, // мантисса 1234567, scale 7
...
TO_FLOAT_TEST_CASES(to_float_pure_zero, {
  .src = {{0x00000000, 0x00000000, 0x00000000, 0x00000000}}, // 0
...
TO_FLOAT_TEST_CASES(to_float_minus_zero, {
  .src = {{0x00000000, 0x00000000, 0x00000000, 0x80000000}}, // -0
  // Проверяет, что аппаратное сохранение знака минус-ноль отработает корректно
  .expected_result = -0.0f,
  .expected_return_code = 0,
...
TO_FLOAT_TEST_CASES(to_float_max_decimal, {
  .src = {{0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0x00000000}}, // MAX_DECIMAL
  // 79228162514264337593543950335.0 в формате float округлится до 7.922816e+28
...
TO_FLOAT_TEST_CASES(to_float_max_scale, {
  .src = {{0x00000001, 0x00000000, 0x00000000, 0x001C0000}}, // 1e-28
  .expected_result = 1e-28f,
  .expected_return_code = 0,
  .test_name = "Min decimal value 1e-28 to float"
})

TO_FLOAT_TEST_CASES(to_float_max_mantissa_max_scale, {
  .src = {{0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0x001C0000}}, // MAX_DECIMAL with scale 28
...
TO_FLOAT_TEST_CASES(to_float_high_bits_only, {
  .src = {{0x00000000, 0x00000000, 0x00000001, 0x00000000}}, // 2^64
...
TO_FLOAT_TEST_CASES(to_float_half_round_up, {
  .src = {{0x001E8481, 0x00000000, 0x00000000, 0x00060000}}, // 2000001 with scale 6 -> 2.000001
...
TO_FLOAT_TEST_CASES(to_float_half_round_down, {
  .src = {{0x001E847F, 0x00000000, 0x00000000, 0x00060000}}, // 1999999 with scale 6 -> 1.999999
  .expected_result = 1.999999f,
  .expected_return_code = 0,
  .test_name = "Float rounding check down"
})

TO_FLOAT_TEST_CASES(to_float_large_with_scale, {
  .src = {{0xFFFFFFFF, 0x000000FF, 0x00000000, 0x00050000}}, // large 64-bit mantissa with scale 5
  .expected_result = 10995116.0f,
  .expected_return_code = 0,
  .test_name = "Large 64-bit mantissa with scale 5 to float"
})

TO_FLOAT_TEST_CASES(to_float_small_negative_fraction, {
  .src = {{0x00003039, 0x00000000, 0x00000000, 0x800B0000}}, // -12345 with scale 11 -> -0.00000012345
