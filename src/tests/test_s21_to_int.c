...
TO_INT_TEST_CASES(to_int_with_truncate,
                  {.src = {{0x0000007B, 0x00000000, 0x00000000,
                            0x00020000}},  // 1.23 (scale 2)
                   // Дробная часть .23 должна просто отсечься, в результат
                   // пойдет чистая единица
                   .expected_result = 1,
                   .expected_return_code = 0,
                   .test_name = "Truncate fractional part: 1.23 to 1"})

TO_INT_TEST_CASES(to_int_max_limit,
                  {.src = {{0x7FFFFFFF, 0x00000000, 0x00000000,
                            0x00000000}},  // INT_MAX
...
TO_INT_TEST_CASES(to_int_min_limit,
                  {.src = {{0x80000000, 0x00000000, 0x00000000,
                            0x80000000}},  // INT_MIN (-2147483648)
                   // Проверяет, что модуль 0x80000000 со знаком минус корректно
                   // собрался в знаковый int
...
TO_INT_TEST_CASES(to_int_overflow_error,
                  {.src = {{0x80000000, 0x00000000, 0x00000000,
                   ...
                   // Это число превышает INT_MAX на единицу, должно возвращать
                   // ошибку конвертации (1)
                   .expected_result = 0,
                   .expected_return_code = 1,  // ERROR
                   .test_name = "Positive overflow error: 2147483648"})

TO_INT_TEST_CASES(
    to_int_huge_overflow_error,
    {.src = {{0x00000000, 0x00000001, 0x00000000,
              0x00000000}},  // 2^32 (единица в bits[1])
     // Число занимает больше одного инта мантиссы, гарантированная ошибка
     // конвертации (1)
     .expected_result = 0,
     .expected_return_code = 1,  // ERROR
