...
DIV_TEST_CASES(div_by_one_preserves_scale,
               {.value1 = {{0x0000007B, 0x00000000, 0x00000000,
                            0x00020000}},  // 1.23 (scale 2)
                .value2 = {{0x00000001, 0x00000000, 0x00000000,
                            0x00000000}},  // 1 (scale 0)
                .expected_result = {{0x0000007B, 0x00000000, 0x00000000,
                                     0x00020000}},  // 1.23 (scale 2 - 0 = 2)
                .expected_return_code = 0,
...
DIV_TEST_CASES(div_bankers_round_up,
               {// 3e-28 / 2 = 1.5e-28 -> в алгоритме превратится в 15e-29.
                // check_result урежет масштаб до 28, получит мантиссу 1 и
                // остаток 5. Нечётная 1 округлится по банковскому правилу вверх
                // до ближайшего чётного — до 2!
                .value1 = {{0x00000003, 0x00000000, 0x00000000,
                            0x001C0000}},  // 3e-28 (scale 28)
                .value2 = {{0x00000002, 0x00000000, 0x00000000,
                            0x00000000}},  // 2 (scale 0)
...
DIV_TEST_CASES(div_overflow_to_infinity,
               {.value1 = {{0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
                ...
                .value2 = {{0x00000001, 0x00000000, 0x00000000,
                            0x00010000}},  // 0.1 (scale 1, мантисса 1)
                // MAX_DECIMAL / 0.1 = MAX_DECIMAL * 10 -> гарантированный взрыв
...
DIV_TEST_CASES(div_overflow_to_negative_infinity,
               {.value1 = {{0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
                ...
                .value2 = {{0x00000001, 0x00000000, 0x00000000,
                            0x00010000}},  // 0.1 (scale 1, мантисса 1)
                // -MAX_DECIMAL / 0.1 = -MAX_DECIMAL * 10 -> гарантированный
                // взрыв в минус бесконечность
                .expected_result = {{0x00000000, 0x00000000, 0x00000000,
                                     0x00000000}},
...
DIV_TEST_CASES(
    ...
    {.value1 = {{0x00000001, 0x00000000, 0x00000000, 0x00000000}},  // 1
     ...
     .expected_result = {{0x94924924, 0x5205497B, 0x049DAFC4,
                          0x001C0000}},  // scale 28
     .expected_return_code = 0,
...
DIV_TEST_CASES(div_scale_jump,
               {.value1 = {{0x00000005, 0x00000000, 0x00000000,
                ...
                .value2 = {{0x00000002, 0x00000000, 0x00000000,
                            0x00020000}},  // 0.02 (scale 2, мантисса 2)
                // 0.5 / 0.02 = 25. Стартовый scale 1 - 2 = -1 -> выравнивается
                // до 0 (мантисса 50). 50 / 2 = 25.
                .expected_result = {{0x00000019, 0x00000000, 0x00000000,
                                     0x00000000}},  // 25 (scale 0)
                .expected_return_code = 0,
                .test_name = "0.5 / 0.02 = 25 (negative scale normalization)"})

DIV_TEST_CASES(
    div_underflow_to_zero,
    {.value1 = {{0x00000001, 0x00000000, 0x00000000,
     ...
     // 1e-28 / 10 = 1e-29 -> scale 29 превышает лимит. check_result делит 1 на
     // 10 -> получаем 0, остаток 1.
     .expected_result = {{0x00000000, 0x00000000, 0x00000000,
                          0x001C0000}},  // 0e-28 (чистый ноль со scale 28)
     .expected_return_code = 0,
