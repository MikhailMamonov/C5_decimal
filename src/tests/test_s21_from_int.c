...
FROM_INT_TEST_CASES(
    from_int_basic_positive,
    {.src = 15,
     .expected_result = {{0x0000000F, 0x00000000, 0x00000000,
                          0x00000000}},  // 15, sign = 0, scale = 0
     .expected_return_code = 0,
     .test_name = "Basic positive int: 15"})

FROM_INT_TEST_CASES(
    from_int_basic_negative,
    {.src = -15,
     .expected_result = {{0x0000000F, 0x00000000, 0x00000000,
                          0x80000000}},  // 15, sign = 1 (0x80000000), scale = 0
     .expected_return_code = 0,
     .test_name = "Basic negative int: -15"})

FROM_INT_TEST_CASES(from_int_minus_one,
                    {.src = -1,
                     // Проверяет, что дополнительный код 0xFFFFFFFF успешно
                     // превратился в чистый модуль 1 со знаком минус
...
FROM_INT_TEST_CASES(
    ...
    {.src =
         -2147483648,  // INT_MIN
                       // Проверяет, что 0x80000000 на беззнаковом уровне устоял
                       // и записался как модуль 2147483648 со знаком минус
